...
(function ($) {
  ...
  Drupal.chatblock.response = function(jsonData) {
    ...
    // Store new token data, if given.
    if (typeof jsonData.tokenData == 'object') {
      Drupal.settings.chatblock.tokenData = jsonData.tokenData;
    }
    ...
    if (jsonData.messages && jsonData.messages.length) {
      ...
      // Remember the updated timestamp.
      Drupal.settings.chatblock.lastTimestamp = jsonData.lastTimestamp;
      Drupal.settings.chatblock.maxId = jsonData.maxId;
